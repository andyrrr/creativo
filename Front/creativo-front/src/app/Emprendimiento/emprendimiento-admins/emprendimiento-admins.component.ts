...
@Component({
  selector: 'app-emprendimiento-admins',
  templateUrl: './emprendimiento-admins.component.html',
  styleUrls: ['./emprendimiento-admins.component.css']
})
export class EmprendimientoAdminsComponent {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.service.add(this.admin).subscribe({
          next:(data) =>{
            this.service.successMessage("Solucitud Enviada", "/emprendimiento-admins")
          }, error:(err) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.service.update(this.admin.IdEntrepreneurship, this.admin).subscribe({
          next:(data) =>{
            this.service.successMessage("Administrador Removido", "/emprendimiento-admins")
          }, error:(err) => {
            this.service.errorMessage(err.error.Message);
          }
        })
  ...
  ReenviarSolicitud(user:string){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.service.update(this.admin.IdEntrepreneurship, this.admin).subscribe({
          next:(data) =>{
            this.service.successMessage("Solucitud Reenviada", "/emprendimiento-admins")
          }, error:(err) => {
            this.service.errorMessage(err.error.Message);
          }
        })
